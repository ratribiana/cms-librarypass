...
export class DualListComponent implements DoCheck, OnChanges {
  ...
  static DEFAULT_FORMAT = { add: 'Add', remove: 'Remove', all: 'Select All', none: 'Deselect All', direction: DualListComponent.RTL, draggable: true };
  ...
  buildConfirmed(destination:Array<any>) : boolean {
    ...
    if (destChanges) {
      ...
      this.selectedCount = this.confirmed.list.length;
