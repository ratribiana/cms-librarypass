...
@Component({
  selector: 'app-auth-classlink',
  templateUrl: './auth-classlink.component.html',
  styleUrls: ['./auth-classlink.component.css']
})
export class AuthClasslinkComponent implements OnInit {
  ...
  ngOnInit() {        
    ...
    this.getTenants(this.page, this.filter);
  ...
  updateLibraryAuthMethod(e) {
    ...
    this.libraryService.updateLibraryAuthMethod(this.library_id, this.auth_id, form_data)
      .subscribe(
        data => {
          this.loading = false;
          this.updateLibraryAuthMethodForm.get('tenant_id').reset();
          this.updateLibraryAuthMethodForm.get('org_id').reset();
          this.getTenants(this.page, this.filter);
  ...
  getTenants(page: number, filter: string = '') {
    this.tenants = this.libraryService.getLibraryAuthClasslinkTenants(this.library_id, page, filter, this.limit)
      .pipe(tap(res => {
  ...
  deleteTenant(e, id: number, tenant_id: number) {
    let row = e.target.parentNode.parentNode;

    let r = confirm(`Delete Tenant ${tenant_id}?`);

    if (r == true) {
      this.libraryService.deleteLibraryAuthClasslinkTenant(this.library_id, id)
        .subscribe(
          data => {
            this._flashMessagesService.show('Successfully Deleted Tenant.', { cssClass: 'alert-success', timeout: 5000 });
            row.remove();
            this.getTenants(this.page, this.filter);            
          },
